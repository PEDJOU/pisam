 ...
 */
@Service
public class ServiceEnvoiSMS_Confirmation {
	...
	private static final Logger logger = LoggerFactory.getLogger(ServiceEnvoiSMS_Confirmation.class);
	...
	public void Sms_Confirmation_RDV() {
		try {
			...
			if (cd != null) {
				...
				List<SmsConfirmationRDV> sms = cmdConfirmation.findByNOK();

				SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");

				for (SmsConfirmationRDV s : sms) {
					...
					String msg = s.getSmsformat().getMessageconfirmation().replace(NOM, s.getNom())
					...
					if (jsonresponse.getStatus() == 200) {
						...
						if (Etat == 1) {
							...
							s.setMessage(msg);
							cmdConfirmation.save(s);
						} else {
							logger.error("Echec d'envoi de SMS De Confirmation de RDV Code ETAT:" + Etat);
						}

					} else {
						logger.error(
								"Echec d'envoi de SMS De Confirmation de RDV Code HTTP:" + jsonresponse.getStatus());
					}
				}

			} else {
				logger.error("Configuration Introuvable - SMS De Confirmation de RDV");
			}
		} catch (Exception ex) {
